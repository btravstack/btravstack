...
describe("readEnv", () => {
  it("falls back to the documented defaults when nothing is set", () => {
    ...
    // THEN all three carry their defaults, the numbers as numbers
  ...
  it("rejects a poll interval of zero where a port's own bounds would allow it", () => {
    // GIVEN a sweep interval that would spin the relay hot
    const source = { OUTBOX_POLL_MS: "0" };

    // WHEN it is validated
    const env = readEnv(source);

    // THEN the deployment's own lower bound speaks, not the shared fragment's
    expect(env).toBeErrWith([expect.objectContaining({ path: ["OUTBOX_POLL_MS"] })]);
  });
});
