/**
 * The compile-time half of the fourth deployment: `orderAmqpRuntime` declares
 ...
 *
 * Together with `order-api`'s, `order-worker`'s and `order-temporal`'s, this is
 * what makes the claim testable rather than asserted: four runtimes with
 * non-empty `needs`, all proven against the same application graph at the
 * `start(...)` call site.
 */
...
const options = {
  runtime: orderAmqpRuntime({ contract: orderContract, urls: ["amqp://127.0.0.1:5672"] }),
  signals: false,
  probes: false,
} as const;
...
const _missingLogger = start(PartialAmqp, options);
