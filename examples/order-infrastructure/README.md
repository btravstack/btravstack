...
prisma/migrations/                 generated from the schema (bar the RLS one), committed, applied by db:migrate
...
`prisma/schema.prisma` and committed — one source of truth for the shape. One
migration is **hand-written**, `20260906120000_order_rls`: Prisma's schema
language has no row security, so regenerating the set would silently drop the
policy `Order` is isolated by. `schema-drift.spec.ts` asserts the policy is
there, which is what makes that loss red.
...
And the database enforces it rather than trusting it: `db` is
`scopedTo(client, tenant)` — `@btravstack/prisma/rls`'s `tenantScoped`, which
pins every statement through `set_config('app.tenant_id', …)` — and `Order`
carries a `tenant_isolation` policy under `FORCE ROW LEVEL SECURITY` reading
that same setting. So `list` names no tenant in its `where` at all; the policy
is what narrows it, and `src/rls.spec.ts` is what proves that against the real
server rather than against a filter.
...
fixture builds `prismaOrderRepository(scopedTo(db, tenant), tenant)`, a second
one over another tenant is what a cross-tenant spec asserts across, and nothing
cleans up.
