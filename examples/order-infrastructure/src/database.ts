 ...
 * configuration rather than read off `process.env` by hand.
 *
 ...
 *
 * `$disconnect()` ends the driver adapter's pool but does not kill the client —
 * Prisma reconnects lazily on the next statement, which is why no spec asserts
 * that a released client refuses to query.
 */
