...
## Why it is not part of `order-temporal`
...
```
   order-temporal        any client starting a workflow
         └──────────┬──────────┘
                    ▼
...
`@btravstack/start-example-order-temporal` under a `@ts-expect-error`, so the
...
"a client built from the contract alone" is what `order-temporal`'s own suite
