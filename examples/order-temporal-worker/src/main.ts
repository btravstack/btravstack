 ...
 * environment — inside the graph, through `TemporalConfig` and the kernel's
 * own probe binding — and a bad one is the kernel's to report: a `startFailed`
 * event and exit `78`. A service that will not answer is `TemporalUnreachable`,
 * exit `1`.
 *
