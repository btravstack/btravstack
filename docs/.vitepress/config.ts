...
export default defineConfig({
  ...
  // time; its cross-references use relative links TypeDoc resolves itself, which
  // is what the three RELATIVE patterns exempt.
  //
  // A fourth pattern, `/^\/api\//`, exempted every ROOT-relative link into the
  // generated tree, and it is gone (issue #191). It was the only thing standing
  // between three dead links in the hub page — `/api/http/`, `/api/temporal/`,
  // `/api/amqp/`, three packages renamed long ago — and a red build, for a year.
  // Measured on removal: those three were the ONLY links it was suppressing, so
  // it cost nothing and hid exactly one bug. `extract-doc-samples.ts` declines to
  // check root-relative links on the grounds that VitePress fails the build on
  // them; that is now true.
