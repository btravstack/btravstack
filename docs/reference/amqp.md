...
| ----------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AmqpModule`            | value | `AmqpModule(name)({ contract, handlers, url?, connectionOptions?, defaultConsumerOptions?, connectTimeoutMs?, tenantOf?, imports?, provides?, exports? })` — a di `Module(name)({...})` that also takes the handlers provider |
...
| `tenantOf`               | no       | —                    | `(args) => string \| undefined` — reads the tenant off a delivery onto `UnitMeta.tenantId`; see [Make an application multi-tenant](/how-to/read-the-ambient-unit#make-an-application-multi-tenant)                                                |
