...
| `@btravstack/prisma`          | `@btravstack/config`, `@btravstack/di`, `unthrown`, `@prisma/adapter-pg` — and **no `@btravstack/core`**: it ships no instrumentation, so it imports nothing from the kernel                                                     |
