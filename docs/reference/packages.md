...
description: The six published packages, who peers on what, and one install command per kind of deployment.
---

# Packages and install

> **Reference.** The six published packages, their peer-dependency matrix and the
...
both; each starter peers on all three plus its own transport library. Nothing
here depends on a runtime package: the kernel knows nothing about HTTP, AMQP or
Temporal.
...
`@btravstack/core`, `@btravstack/config` and `@btravstack/di` have **no
runtime dependencies** beyond `node:` builtins. `@btravstack/amqp` peers on
...
| `@btravstack/core/testing` | `testRuntime`, `TestRuntimePort`, `createFakeClock`, `withApp` — kept out of the main entry so a production bundle never pulls the fakes in; see [Testing entry point](/reference/core/testing) |
...
All six packages ship dual CJS/ESM builds with `.d.ts` files and no source
