...
| `pageRequest`       | root   | value | `<Q extends PageQuery>(query: Q) => (Q extends { readonly sort: Sort<infer F> } ? PageRequest<F> : PageRequest) & Omit<Q, "after" \| "before">` — the crossing, filters and sort carried through; an OPTIONAL `sort` is refused at the call rather than narrowed to the unsorted request                           |
