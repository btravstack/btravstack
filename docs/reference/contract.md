...
| `PageQuery`         | root   | type  | `{ limit: number; after?: string; before?: string; sort?: Sort }` — the flat shape a schema validates to                                                                                                                                                                                                           |
| `pageRequest`       | root   | value | `<Q extends PageQuery>(query: Q) => (Q extends { readonly sort: Sort<infer F> } ? PageRequest<F> : PageRequest) & Omit<Q, "after" \| "before">` — the crossing, filters and sort carried through                                                                                                                   |
