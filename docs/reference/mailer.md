...
| --------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| span      | `mail.send`                                                                           | `btravstack.mail.recipients` and `btravstack.mail.attachments` (counts), `btravstack.mail.subject`; error status on a failure |
| counter   | `btravstack.mail.operations`                                                          | `{ operation, outcome }` — `send`, and `ok` or `error`                                                                        |
| histogram | `btravstack.mail.duration`                                                            | the same as the counter                                                                                                       |
| log       | `"mail.send failed"` at `error`, on a failure only — a successful send writes no line | `operation`, the two counts and the subject, with the failure as the cause                                                    |
