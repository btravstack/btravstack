...
| --------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `UNSATISFIED DEPENDENCIES — nothing provides`       | Something in the graph reads a port nothing provides. The port is named by its **id**.        | `provides`, or an import that provides it. Not `exports`.                                        |
...
| `UNSATISFIED RUNTIME PORTS`                         | The runtime resolves a port the module does not **export**.                                   | `exports`. The module's own, never the unit module's.                                            |
| `UNSATISFIED UNIT NEEDS`                            | `StartOptions.unit`'s module needs a port the application module does not export.             | `exports`, on the application module.                                                            |
