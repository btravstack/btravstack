...
| `HTTP_SESSION_KEYS`        | required                | [`sessionCodec()`](/reference/http-server#the-session-cookie) — comma-separated 32-byte base64url keys; the first seals, every one unseals                             |
| `HTTP_OIDC_ISSUER`         | required                | [`oidc()`](/reference/http-server#the-login-answerer) — the provider, as its discovery document names itself, a `Config.url` field                                     |
| `HTTP_OIDC_CLIENT_ID`      | required                | `oidc()` — this deployment's client                                                                                                                                    |
| `HTTP_OIDC_CLIENT_SECRET`  | required                | `oidc()` — its secret: the authorization-code flow here is a confidential client's                                                                                     |
| `HTTP_OIDC_REDIRECT_URI`   | required                | `oidc()` — the URI **registered** with the provider, which is also what the code grant is checked against, a `Config.url` field                                        |
