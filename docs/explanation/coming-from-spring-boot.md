...
SpEL expression is evaluated just before the method runs, and a scope the
scheme cannot grant is refused when the contract compiles. `@PostAuthorize` on the returned
...
result the operation demands, so handing the operation a resource with no
witness is a compile error — a forgotten rule cannot ship — where a method
whose proxy was bypassed skips its annotation silently. The
