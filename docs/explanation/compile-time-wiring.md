...
`start` accepts a `Module<X, E, N>` and asks three questions of it: one di's own
entry points ask too — is anything unprovided — and two about `X` that di has
no reason to ask. They arrive as a phantom marker named
`StartGate<X, N>`, **intersected onto the `module` parameter** —
`unknown`, and invisible, when the gate is satisfied; a diagnostic otherwise.
`start`, `runMain` and `@btravstack/testing`'s `Boot` all carry it:

| Arm                         | Fires when                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `UNSATISFIED DEPENDENCIES`  | Something in the graph needs a port nothing provides — checked **first**, so the mistake nearest the root prints rather than a consequence of it, and answered in di's own sentence rather than one the kernel invented. It ends on the port's **id** (`"OrpcRouter"`, `"AmqpHandlers"`), which is what the application wrote in `Port("…")`. `Scope` and `Env` are excluded: those two the kernel discharges itself. A bound `unit` module's own unmet needs join this same arm — they travel in the starter's own `Needs` channel, exactly like an import's — so there is no separate arm for them. |
...
| `UNSATISFIED RUNTIME PORTS` | The runtime's declared `resolves` are not among the module's exports — the **module's alone**, never a fork's, because `RuntimeHost.ctx` is the application context and a unit-only port does not exist at startup. `@btravstack/http-server` declares one: `HttpRuntime` resolves `HttpHandler`, so a root that does not export it is refused here — `HttpModule` adds it to `exports` itself.                                                                                                                                                                                                       |
...
Reading one of these is a small skill of its own — the sentence prints last,
past your own contract — and it has a page:
...
| An unmet need at `start` (`StartGate`'s first arm, since issue #203)     | a starter's own port, e.g. `AmqpHandlers`                                             | `'"AmqpHandlers"' is not assignable to type '"@di/Scope"'`                                                                                 | ends on `'{ readonly "UNSATISFIED DEPENDENCIES — nothing provides": "AmqpHandlers"; }'` — di's own sentence, and the port named by its id. What it replaced was not a bad diagnostic so much as a private one: `@di/Scope` is a phantom nobody outside this repository has heard of, and when the port was ALSO missing from `exports` the marker that fired was `UNSATISFIED RUNTIME PORTS` — a correct diagnosis of the second mistake that reads as a wrong one of the first |
| `start`'s `StartGate` — `NO RUNTIME`                                     | [the Greeter example above](#the-kernel-s-own-gate)                                   | `Expected 4 arguments, but got 1.`                                                                                                         | ends on `"NO RUNTIME — the module exports no port declared over RuntimePort"` — [full example above](#the-kernel-s-own-gate)                                                                                                                                                                                                                                                                                                                                                    |
...
| amqp's/temporal's composer — `UNCOVERED HANDLERS`/`UNCOVERED ACTIVITIES` | `AmqpHandlers(contract)([...])` / `TemporalActivities(contract)([...])` missing a key | ends on `'"UNCOVERED HANDLERS"'` / `'"UNCOVERED ACTIVITIES"'`                                                                              | ends on `'"UNCOVERED HANDLERS — the contract declares a consumer this array does not cover"'` / the `ACTIVITIES` twin; the missing key prints beside the marker on the trailing element, at every arity, because the refusal is a tuple as long as the array the caller wrote (measured on a one-element array: `'readonly ["UNCOVERED HANDLERS — …", "right"]'`, `'readonly ["UNCOVERED ACTIVITIES — …", "audit" \| "runShout"]'`)                                             |
| http's composing router — `UNCOVERED CONTROLLERS`                        | `api.OrpcRouter(contract)([...])` missing a piece                                     | (the retired keyed record's own gate) ends on `'"UNDECLARED KEY — the contract declares no fragment under billing"'`                       | ends on `'"UNCOVERED CONTROLLERS — the contract declares a procedure this array does not cover"'` — the missing procedures print beside the marker on the trailing element, at every arity (measured on a one-element array: `'readonly ["UNCOVERED CONTROLLERS — …", "billing.pay" \| "users.find"]'`)                                                                                                                                                                         |
| http's composing router — a piece whose own mint was refused             | `api.OrpcRouter(contract)([ok, HttpController(contract, "typo")])`                    | a false `OVERLAPPING CONTROLLERS`, since the refused mint's key reads as every valid path at once — first error right, loudest error wrong | no second marker: both array gates stand down when an element's key is a union, and the mint's own `TS2345` — which lists every valid path — is the diagnostic to read                                                                                                                                                                                                                                                                                                          |
| http's composing router — `OVERLAPPING CONTROLLERS`                      | `api.OrpcRouter(contract)([...])` with a piece nested inside another piece's path     | (no equivalent — the retired keyed record had no nested paths to overlap)                                                                  | ends on `'"OVERLAPPING CONTROLLERS — a piece sits inside another piece's fragment"'`                                                                                                                                                                                                                                                                                                                                                                                            |
