...
`start` accepts a `Module<X, E, N>` and asks four questions of it: one di's own
entry points ask too — is anything unprovided — and three about `X` that di has
no reason to ask. They arrive as a phantom marker named
`StartGate<X, UnitNeeds, N>`, **intersected onto the `module` parameter** —
...
| `UNSATISFIED RUNTIME PORTS` | The runtime's declared `resolves` are not among the module's exports — the **module's alone**, never the unit module's, because `RuntimeHost.ctx` is the application context and a unit-only port does not exist at startup. `@btravstack/http-server` declares one: `HttpRuntime` resolves `HttpHandler`, so a root that does not export it is refused here — `HttpModule` adds it to `exports` itself.              |
