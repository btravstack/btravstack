 ...
 * root composing a store and no observability fails di's dependency gate naming
...
// modules is a compile error naming the ports, not a storage that silently
// counts nothing.
...
const _plain = Module.scoped(Plain, (ctx) => OkAsync(ctx.get(Storage)));
...
const Dynamic = Module("Dynamic")({
  imports: [storage({ adapter: memoryStorage(), instrumented: decided })],
  exports: [Storage],
});

// @ts-expect-error — UNSATISFIED DEPENDENCIES: a maybe-instrumented storage owes the three ports.
const _dynamic = Module.scoped(Dynamic, (ctx) => OkAsync(ctx.get(Storage)));
