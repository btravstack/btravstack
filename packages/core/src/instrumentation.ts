 ...
 * on — the same reason `Tracer` and `Meter` are declared as narrowings rather
 * than as OTel's own types. The collector
 * (`@btravstack/observability/otel`) is where the vendor is already a
 * dependency, so that is where the cast belongs.
 */
export type InstrumentationLoader = () => Promise<unknown>;
 ...
 * shape, in one port.
 */
export class Instrumentations extends Port.many("Instrumentations")<InstrumentationLoader> {}
