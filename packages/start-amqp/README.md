...
kernel unit per delivery through a middleware, and — unlike `-temporal`,
...
unlike `-temporal`'s single `declareActivitiesHandler` call, this is **two**
...
`-temporal` gets to make. `amqp-contract`'s own dispatch routes a modeled
...
`drainTimeoutMs: null` is what makes this package simpler than `-temporal`:
...
survives every case, the same answer `-http` reaches per request.
