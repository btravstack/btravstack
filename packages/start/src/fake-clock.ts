...
export type FakeClock = Clock & {
   ...
   *
   * The returned promise settles once the code under test has had a chance to
   * react — so `await clock.advance(5_000)` leaves the application where the
   * elapsed time takes it, with no extra flushing at the call site.
   */
