...
//    missing key itself is named only once the array's length matches the
...
// The do-not-break property: a slice lifts out of the composed router with its
// PIECE UNCHANGED — the lifted root declares the very provider the modulith
// composed and hands back what it built. Naming the piece is deliberate: a
// fresh `sync` over the fragment would pin only that a fragment is a valid
// contract, which says nothing about the piece surviving the lift.
const _lifted = publicApi.HttpRouter(contract.orders)(
