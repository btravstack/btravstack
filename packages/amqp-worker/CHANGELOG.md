...
- 89d67b8: `connectionOptions` and `defaultConsumerOptions` are typed by
...
- f9d48ec: **Breaking.** `@btravstack/amqp-worker` becomes a starter, the same shape as
  `@btravstack/http-server`'s `http()`. `amqp({ contract, url?, ... })`
