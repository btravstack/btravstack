...
`@btravstack/amqp` boots against **one RabbitMQ container for the whole gate**
rather than one per vitest run, and `@btravstack/temporal` against **one
...
`@btravstack/temporal` no longer downloads a 64 MB binary on a cold cache.
